...
/**
 * Ssrini Handcrafts - Product Data API
 * Handles product listing, search, category filtering and sorting.
 */
...
try {
    ...
    $sql = "
        SELECT
            ...
            p.description, p.price, p.discount_price, p.stock_quantity,
            p.image, p.status, p.created_at, p.updated_at,
    ...
    /* SEARCH (FIXED UNIQUE PLACEHOLDERS) */
    if ($search !== '') {
    ...
    if ($stockFilter === 'out') {
        ...
        $sql .= " AND p.stock_quantity BETWEEN 1 AND 5";
